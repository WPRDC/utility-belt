...
def main():
    ...
    source_type, source_id = investigate_id(site,source_id,API_key)

    destination_id = None
    if len(sys.argv) > 2:
        destination_id = sys.argv[2]

    question = "Are you sure you want to clone {} {}".format(source_type, source_id)
    if source_type != 'package':
        if destination_id is None:
            question += " to a completely new package?"
        else:
            question += " to destination ID {}".format(destination_id)
